...
fixtures and live results from **thesportsdb** automatically and scores
...
- **Auto-sync from thesportsdb**: fixtures refresh every ~6h, results
...
- **No API signup required.** Live data comes from thesportsdb.com via
  their free public test key `"3"`. If you want higher rate limits or
  real in-play minute counters, support them on Patreon for a dedicated
  key — https://www.thesportsdb.com/premium
...
4. Then run each migration in [`supabase/migrations/`](./supabase/migrations/)
   in filename order — they add the thesportsdb integration columns and
...
- `SPORTSDB_KEY` — *optional*. thesportsdb's free public test key `"3"` is
  the default and works without signup; auto-sync runs out of the box.
  Set this only if you have a Patreon-backed dedicated key (higher rate
  limits + real in-play minute data).
- (optional) `SPORTSDB_LEAGUE_ID` / `SPORTSDB_SEASON` — defaults to
  `4429` / `2026`, which is the World Cup on thesportsdb. Override if they
  ever renumber.
...
times. As soon as the app talks to thesportsdb for the first time, the
...
pages will lazily kick off a background sync from thesportsdb (rate
...
- `/admin` — trigger a manual sync from thesportsdb, override results,
...
3. Under **Environment Variables**, add the same `DATABASE_URL` you used
   locally (and `SPORTSDB_KEY` if you set one — otherwise leave it out and
   the public test key `"3"` is used automatically).
...
src/lib/footballData.ts          # Thin thesportsdb v4 wrapper
...
6-match round-robin per group with synthetic kickoff times. The auto-sync
talks to thesportsdb.com on every page render (rate-limited) and replaces
all of that with the real draw and schedule. The sync matches teams by
